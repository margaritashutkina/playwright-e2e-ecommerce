...
environment. GitHub-hosted runners share IP ranges that the platform's
bot protection rate-limits, so CI runs can fail on page load even when
the full suite passes locally. Worker count is capped to reduce this.
Local runs are the source of truth; a staging or preview environment
